...
sensors_poll_context_t::sensors_poll_context_t()
{
        ...
        if((seStatus[ID_A].isUsed == true) && (seStatus[ID_A].isFound == true)) {
                first = first + 1;
                accel = first;
                mSensors[first] = new Lis3dhAccel();
        ...
        if((seStatus[ID_L].isUsed == true) && (seStatus[ID_L].isFound == true)) {
                first = first + 1;
                light = first;
                mSensors[first] = new jsa1127Light();
...
#ifdef DEBUG_SENSOR
        ALOGD("accel = %d,mag = %d,gyro = %d,light = %d,proximity = %d,orn = %d,temperature = %d,press = %d \n",
                accel, mag, gyro, light, proximity, orn, temperature, press);
#endif
}
...
int sensors_poll_context_t::activate(int handle, int enabled) {
	...
	if(handle == ID_O || handle ==  ID_M){
		err =  mSensors[accel]->setEnable(handle, enabled);// if handle == orientation or magnetic, please enable ACCELERATE Sensor
		if(err)
			return err;
	}
...
int sensors_poll_context_t::pollEvents(sensors_event_t* data, int count)
{
        ...
        do {
		        ...
		        do {
			        n = poll(mPollFds, number, nbEvents ? 0 : -1);
